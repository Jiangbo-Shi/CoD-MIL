...
parser.add_argument('--model_type', type=str, choices=['clam_sb', 'clam_mb', 'mil', 'patch_gcn', 'dgc', 'transFG', 'transMIL', 'ABMIL', 'DSMIL', 'ZoomMIL', 'GT', 'CoT', 'AddMIL', 'MaskCLIP'], default='clam_sb',
...
def main(args):
    ...
    for i in folds:
        ...
        results, test_auc, val_auc, test_acc, val_acc, each_class_acc, test_f1, test_patch_acc, test_patch_f1, A = train(datasets, i, args)
        all_test_auc.append(test_auc)
        all_val_auc.append(val_auc)
        all_test_f1.append(test_f1)
        all_test_acc.append(test_acc)
        all_val_acc.append(val_acc)
        all_test_patch_acc.append(test_patch_acc)
        all_test_patch_f1.append(test_patch_f1)
        ...
        # torch.save(A, args.results_dir+'/'+'attention_'+str(i)+'.pt')


    final_df = pd.DataFrame({'folds': folds, 'test_auc': all_test_auc, 'test_acc': all_test_acc, 'test_f1': all_test_f1, 'test_patch_acc': all_test_patch_acc, 'test_patch_f1': all_test_patch_f1})
    result_df = pd.DataFrame({'metric': ['mean', 'var'],
                              ...
                              'test_patch_f1': [np.mean(all_test_patch_f1), np.std(all_test_patch_f1)],
                              'test_patch_acc': [np.mean(all_test_patch_acc), np.std(all_test_patch_acc)]
