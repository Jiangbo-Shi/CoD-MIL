...
parser.add_argument('--model_type', type=str, choices=['clam_sb', 'clam_mb', 'mil', 'patch_gcn', 'dgc', 'transFG', 'transMIL', 'ABMIL', 'DSMIL', 'ZoomMIL', 'GT', 'GraphMIL', 'CoT', 'AddMIL', 'MaskCLIP'], default='clam_sb',
                    help='type of model (default: clam_sb)')
parser.add_argument('--mode', type=str, choices=['clam', 'graph', 'transformer'], default='clam', help='Specifies which modalities to use / collate function in dataloader.')
...
parser.add_argument("--text_prompt_feature_path", type=str, default=None,
                    help="pretrained path")
...
if args.task == 'task_1_tumor_vs_normal':
...
elif args.task == 'task_subtyping_tfah_subtyping':
    args.n_classes=3
    dataset = Generic_MIL_Dataset(csv_path = 'dataset_csv/TFAH_RCC_subtyping.csv',
                                  mode = args.mode,
                                  data_dir_s = os.path.join(args.data_root_dir, args.data_folder_s),
                                  data_dir_l = os.path.join(args.data_root_dir, args.data_folder_l),
                                  shuffle = False,
    ...
    dataset = Generic_MIL_Dataset(csv_path = 'dataset_csv/TCGA_BRCA_subtyping.csv',
                                  ...
                                  patient_strat= False,
                                  ignore=[])
elif args.task == 'task_tcga_camelyon16':
    args.n_classes=2
    dataset = Generic_MIL_Dataset(csv_path = 'dataset_csv/Camelyon16_subtyping.csv',
                                  mode = args.mode,
                                  data_dir_s = os.path.join(args.data_root_dir, args.data_folder_s),
                                  data_dir_l = os.path.join(args.data_root_dir, args.data_folder_l),
                                  shuffle = False,
                                  print_info = True,
                                  label_dict = {'cancer':0, 'normal':1},
                                  patient_strat= False,
                                  ignore=[])                                
...
if __name__ == "__main__":
    ...
    for ckpt_idx in range(len(ckpt_paths)):
        ...
        model, patient_results, test_error, auc, test_f1, df, each_class_acc, attention_result, test_patch_error, test_patch_f1 = eval(args.mode, split_dataset, args, ckpt_paths[ckpt_idx])
        all_results.append(all_results)
        all_auc.append(auc)
        all_acc.append(1-test_error)
        all_f1.append(test_f1)
        all_patch_acc.append(1-test_patch_error)
        all_patch_f1.append(test_patch_f1)
    ...
    # confusion = np.array(confusion_matrix(np.array(all_true).astype(np.int), np.array(all_pred).astype(np.int)))
    # plot_confusion_matrix(confusion, ['pt1', 'pt2', 'pt3&4'], os.path.join(args.save_dir, 'confusion_matrix.png'))

    final_df = pd.DataFrame({'folds': folds, 'test_auc': all_auc, 'test_f1': all_f1, 'test_acc': all_acc, 'test_patch_f1': all_patch_f1, 'test_patch_acc': all_patch_acc})
    result_df = pd.DataFrame({'metric': ['mean', 'var'],
                              ...
                              'test_patch_f1': [np.mean(all_patch_f1), np.std(all_patch_f1)],
                              'test_patch_acc': [np.mean(all_patch_acc), np.std(all_patch_acc)]
